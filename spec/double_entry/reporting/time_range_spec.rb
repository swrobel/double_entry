  ...
  module Reporting
    RSpec.describe TimeRange do
      it 'should correctly calculate a month range' do
        ...
        expect(ar.start.to_s).to eq Time.zone.local(2009, 10, 1, 0, 0, 0).to_s
        expect(ar.finish.to_s).to eq Time.zone.local(2009, 10, 31, 23, 59, 59).to_s
      end
      ...
      it "should correctly calculate the current week range for New Year's Day" do
        Timecop.freeze Time.zone.local(2009, 1, 1) do
